...
.data 
    ...
    PL      equ 0FEh      ;дизайн плиток             
   
    LEFT    equ 4B00h     ;расширенный код "<-"
    RIGHT   equ 4D00h     ;расширенный код "->"  
    EENTER   equ 1C0Dh    ;расширенный код "Enter"  
 ...
 main:                 
  ...
  cycle:  
    ...
    begin:
        ...
        mov ax, [size_platform]  ;записываем в ax размер платформы          
        add [platformRight],ax   ;правая позиция платформы         
        ret
     go:
     ...
     cursorHide:                  
        ...
        mov cx, 2000h   ;делаем курсор незаметным        
        int 10h 
        ret
     drawScore:
        mov di, 00h           
     ...
     cycle_:                 
        ...
        rep movsb                  ;записываем в ячейку по адресу es:di байт из ячейки с адресом ds:si (пока cx станет равным 0)
     ...
     new_line:                   
        add ax, [size_line] ;добавляем размер следующей строки   
     ...
     step:
        ...
        sub bx, [size_line] ;в bx добавляем длину линии для дальнейшего сравнения с ax     
        jmp loopl           ;начинаем рисовать следующую линию
     drawBlock:               ;блок из 1 элемента  
        push cx               ;запоминаем кол-во кирпичиков
        mov cx, 0004h         ;кол-во выемок в кирпичике   
     drawBlock2:              ;рисуем кирпич      
        mov word ptr es:[bx], PL       ;получаем элемент, который потом будет закрашен
        mov word ptr es:[bx+1], 015h   ;получаем цвет кирпичика 
        add bx, 2             ;добавляем 2(т.к. мы сделали два действия), чтобы потом сравнить с ax   
        dec cx                ;уменьшаем счётчик
        cmp cx, 0             ;проверяем, сколько отверстий нарисованно    
     ...
     drawSpace2:
        mov word ptr es:[bx], SPA    ;заполняем пробелом    
        mov word ptr es:[bx+1], 0h   ;получаем цвет (чёрный)
        add bx, 2           ;добавляем 2(т.к. мы сделали два действия), чтобы потом сравнить с ax
        dec cx              ;проверяем сколько мы заполнили места
        cmp cx, 0           ;проверяем сколько мы заполнили места
     ...
     go_right:                         ;двигаем вправо   
        ...
        add bx, [size_platform]        ;добавляем к bx размер платформы и получаем правую позицию платформы  
        cmp bx, [endLine]              ;если мы перешли правую границу окна, то не меняем позицию, 
        jge movePlatform               ;а идем на проверку передвижения платформы
        mov word ptr es:[bx],PL                 ;добавляем элемент справа, который потом будет закрашен
        mov word ptr es:[bx+1], 011h            ;закрашиваем элемент и остальную область 
        mov bx, [platformLoc]         		 ;получаем левую позицию платформы
        mov word ptr es:[bx],SPA                ;очищаем место, которое нам уже не надо
        mov word ptr es:[bx+1],0h               ;и закрашиваем чёрным цветом
     ...
     go_left:                       ;двигаем влево    
        ...
        jle movePlatform            ;а идем на проверку передвижения платформы 
        ...
        mov word ptr es:[bx+1],0h            ;и закрашиваем чёрным цветом
        mov bx, [platformLoc]       ;получаем левую позицию платформы     
        mov word ptr es:[bx],PL              ;добавляем элемент слева, который потом будет закрашен
        mov word ptr es:[bx+1], 011h         ;закрашиваем элемент и остальную область
        jmp movePlatform            ;переходим на проверку движ. платформы
     drawBall:
        ...
        mul [size_line]          ;ax умножаем на ширину
        ...
        je return             
        mov di, ax               ;заносим нашу позицию в di
        mov si, offset ball      ;занесли адрес смещения для рисования мячика в si
        mov cx, 2                ;нарисовать надо две вещи - сам шарик и цвет
        cld                      ;очистка флага направления
        rep movsb                ;рисуем
        mov word ptr es:[bx], SPA         ;поле смены позиции, когда мячик начал движение
        mov word ptr es:[bx+1], 0h        ;убираем/очищаем предыдущую позицию
        ret     
        
     ;изменяем направление вектора Y
     changeVectorY:          
        neg [vectorY]            
        jmp checkBorderX  
     ;изменяем направление вектора X
     changeVectorX: 
        neg [vectorX]
        jmp next  
     ...
     ;проверка на попадание в кирпич и на поражение
     next2:  
        pop di
        mov ax, es:[di]
        cmp al, PL        ;если попал в кирпич
        je back_move      ;переходим на проверку попадания в кирпич и движение назад
     ...
     loop1:                 ;проверяем попали мы именно в кирпич          
        sub bx, 2           ;отнимаем от bx 2      
        cmp bx, ax
        jl go1
        cmp word ptr es:[bx], SPA    ;проверяем на пробел
        jne loop1           ;пока не будет проверен весь кирпич
     go1:                       
        add bx, 2           ;добавляем к bx 2, т.е. мы перешли в loop1 на 2   
        call drawSpace
        add [points],10     ;добавляем за каждый сбитый кирпич по 10      
        call points_show    ;рисуем кол-во очков в score
     ...
     check_go_awake: 
        ...
        add bx, [vectorX]     ;добавляем vectorX(смещение мячика по Ox)   
        mov ax, es:[bx]       ;заносим в видеопамять  
        cmp al, 0FEh          ;проверяем, где мяч
     ...
     ;обработка исключительных ситуаций при попадании в кирпич(сразу два кирпича, 
     ;если мы в верхней границе, т.е. пока не выйдем от туда убирать все кирпичики, 
     ;в которые попадает мячик)
     loop3:                  
        sub bx, 2
        cmp bx, dx
        je go2
...
points_show:                         
    ...
    je win                  ;если да, то мы выиграли, иначе - рисуем набранное кол-во очков
    lea bx, points          
...
division:
    ...
    inc cx          ;увеличиваем кол-во символов, которое в итоге будет
    cmp ax, 0       ;пока не 0
    jne division    
...
save_in_str:
    ...
    mov byte ptr [di], dl        ;добавляем в адрес points_str
    inc di              ;увеличиваем di
    mov byte ptr [di], 0Fh       ;зарисовываем
    inc di              ;увеличиваем di
...
endProgram:  ;Завершение игры и программы
    call clearScreen
    mov ax, 4C00h
    int 21h
