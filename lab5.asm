...
strlen macro stroke						;длина строки
	push di 
	push cx
	mov al, 0
...
move_fptr macro pcx, pdx, pal, psi 		;pcx*65536+pdx - смещение, pal - откуда перемещение в файле (0 - начало,1 - текущее положение, 2 - конец), psi - с каким файлом работа
	push ax 
	push dx 
	push cx
	push si
...
shift_fptr proc 						;si - [0-handle, 1-handle_temp]
	push bx 
	mov ah, 42h
	cmp si, 0
	jne sfptr_temp
...
write_change proc 						;запись новой подстроки
	push ax 
	push bx
	push dx
	push cx
...
parse_cmd proc 							;читаем путь файла, а также необходимые подстроки
    ...
    xor cx,cx
    	...
    	mov cl, ds:[80h] 					;длина командной строки
	cmp cl, 7
	ja cmdl_parse_continue
	mov ds, ax
	print_str not_enough_cmdl_len_str 	                 ;если строки командной строки слишком короткие
	print_str cmd_line_example_str
	jmp bend
	cmdl_parse_continue:
	mov di, offset cmd_line
	mov si, 82h  
	rep movsb  							;читаем все аргументы командной строки
	...
	find_param:
		mov al, ' '
		cmp [di], al 					;пропускаем пробел
		jne before_read_param
		inc di
		jmp find_param 					;если символ не пробел
	before_read_param:
		inc argc
	read_param:
		cmp [di], byte ptr 0dh 			;выходим если конец аргументов
		je params_end
		cmp [di], byte ptr ' ' 			;если пробел, то читаем подстроки
		je param_readen

		mov al, [di]					
		mov [si], al				;записываем путь файла в строку
		inc di
		inc si
		jmp read_param

	param_readen:   					;когда путь прочитан
		inc di   
...
start:
	...
	before_strlen:								;длина подстроки для замены
	strlen string 
	cmp ax, 0									;проверяем на пустоту первую подстроку
	jne find_len_continue	
	print_str input_error_str
	jmp bend
	find_len_continue:							
	mov string_len, ax
	strlen change 								;длина подстроки, на которую замена
	mov change_len, ax

	call open_file	
	call open_file_temp

	next_buffer: 								;подсчет количества байт в файле
		read_buffer_from 0
		mov cx, buffer_len
		cmp cx, string_len						;если длина буфера меньше длины подстроки
		jge before_scasb
	...
	before_scasb:
		...
		repne scasb
		cmp cx, 0
		jne after_scasb 						;если найден первый символ подстроки
