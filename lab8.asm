...
getVector MACRO oldHandler
    ...
    INT 21h
    MOV WORD PTR oldHandler, BX			;смещение обработчика прерывания
    MOV WORD PTR oldHandler + 2, ES		;сегментный адрес обработчика прерывания
ENDM

setVector MACRO newHandler
    MOV AH, 25h					;установить адрес обработчика прерывания
    MOV DX, OFFSET newHandler			;смещение обработчика в сегменте
    INT 21h       
ENDM
...
puts MACRO string size 
    ...
    MOV AH, 13h					;писать строку
    MOV AL, 1					;использовать атрибут в BL; курсор - в конец строки
...
fileCloser MACRO file				;закрыть файл
    MOV BX, file
    MOV AH, 3Eh  
    INT 21h   
ENDM
...
new_hand0 PROC FAR
    ...
    MOV AH, 25h					;установить адрес обработчика прерывания
    MOV AL, 08h    				;номер прерывания (интервальный таймер)
    MOV DX, CS:old_hand0			;смещение обработчика в сегменте
    MOV DS, CS:old_hand0 + 2			;сегментный адрес обработчика
    INT 21h
    
    MOV AH, 25h					;установить адрес обработчика прерывания
    MOV AL, 09h    				;номер прерывания (прерывание от клавиатуры)
    MOV DX, CS:old_hand1			;смещение обработчика в сегменте
    MOV DS, CS:old_hand1 + 2			;сегментный адрес обработчика
    INT 21h
...
new_hand1 PROC FAR
    ...
    PUSHF
    CALL CS:DWORD PTR old_hand1
    
    MOV AH, 01h     ; kbhit
    INT 16h
    MOV BH, AH		;сканкод
    JZ dontPressKey
    MOV AH, 02h		;читать состояние shift-клавиш
    INT 16h
    AND AL, 2       	;shift
    CMP AL, 0
    JE dontPressKey
